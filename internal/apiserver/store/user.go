...
type UserStore interface {
	...
	Delete(ctx context.Context, opts *db.Options) error
	Get(ctx context.Context, opts *db.Options) (*model.User, error)
	List(ctx context.Context, opts *db.Options) (int64, []*model.User, error)
...
func (s *userStore) Create(ctx context.Context, obj *model.User) error {
	if err := s.store.DB(ctx).Create(&obj).Error; err != nil {
		...
		return errorsx.ErrDBWrite.WithMessage("%s", err.Error())
	}
...
// Delete 根据条件删除用户记录.
func (s *userStore) Delete(ctx context.Context, opts *db.Options) error {
...
// Get 根据条件查询用户记录.
func (s *userStore) Get(ctx context.Context, opts *db.Options) (*model.User, error) {
	var obj model.User
	if err := s.store.DB(ctx, opts).First(&obj).Error; err != nil {
		...
		return nil, errorsx.ErrDBRead.WithMessage("%s", err.Error())
	}
...
func (s *userStore) List(ctx context.Context, opts *db.Options) (count int64, ret []*model.User, err error) {
	...
	if err != nil {
		...
		err = errorsx.ErrDBRead.WithMessage("%s", err.Error())
	}
	return
}
